...
namespace Intent.Modules.Angular.ServiceProxies.Templates.Proxies;
...
public static class ServiceMetadataQueries
{
    ...
    private static bool AzureFunctionIsHttpTrigger(IHttpEndpointModel endpoint)
    {
        return endpoint.InternalElement.GetStereotype(StereotypeAzureFunction)?.GetProperty<string>("Type") == "Http Trigger";
    }

    public static bool HasJsonWrappedReturnType(IHttpEndpointModel endpoint)
    {
        if (endpoint.MediaType != null)
        {
            return endpoint.MediaType == HttpMediaType.ApplicationJson;
        }
