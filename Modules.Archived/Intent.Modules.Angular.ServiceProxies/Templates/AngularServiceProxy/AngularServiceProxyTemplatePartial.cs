...
using Intent.Modules.Angular.ServiceProxies.Api;
...
namespace Intent.Modules.Angular.ServiceProxies.Templates.AngularServiceProxy
{
    ...
    public partial class AngularServiceProxyTemplate : TypeScriptTemplateBase<Intent.Modelers.Types.ServiceProxies.Api.ServiceProxyModel>, ITypescriptFileBuilderTemplate
    {
        ...
        public const string TemplateId = "Intent.Angular.ServiceProxies.AngularServiceProxy";
        ...
        public AngularServiceProxyTemplate(IOutputTarget outputTarget, Intent.Modelers.Types.ServiceProxies.Api.ServiceProxyModel model) : base(TemplateId, outputTarget, model)
        {
            ...
            var (moduleElement, moduleName) = Model.GetModule(ExecutionContext);
            var relativeLocation = moduleElement != null
                ? $"{string.Join("/", moduleElement.GetFolderPath(additionalFolders: moduleName.ToKebabCase()))}"
                : this.GetFolderPath();

            TypescriptFile = new TypescriptFile(relativeLocation)
                .AddClass(Model.Name, @class =>
                {
                    @class.Export();
                    @class.AddDecorator(UseType("Injectable", "@angular/core"), decorator => decorator.AddArgument(InjectableHelpers.GetOptions(this)));
      ...
      .pipe({UseType("map", "rxjs/operators")}((response: {GetApiResponseType(endpoint)}) => {{
        ...
        public override void BeforeTemplateExecution()
        {
            ...
            var moduleElement = Model.GetModule(ExecutionContext).Element;
